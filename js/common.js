...
$(function() {
    ...
    function initGhost() {
        ...
        if (Math.floor(Math.random() * 2317) != 7) {
            return;
        }
        createGhost();
    }
    ...
    function createGhost() {
        ...
        ghostFlying = $('<img/>').attr("src", tools.replace(/\/index.*$/, '/img/toys/' + src + '.png'));
        ghostFlying.css('position', 'absolute').height(ghostH).width(ghostW);
        var body = $('body');
        var h = $(window).height() - ghostH;
        var w = $(window).width() - ghostW;
        ghostX1 = 0;
        ghostX2 = w;
        var top = body.scrollTop();
        ghostY1 = Math.floor(Math.random() * h) + top;
        ghostY2 = Math.floor(Math.random() * h) + top;
        if (Math.random() > 0.5) {
            ghostX1 = ghostX2;
            ghostX2 = 0;
        }
        ghostFlying.css('left', ghostX1).css('top', ghostY1).hide();
        ghostFlying.appendTo($('body'));
        ghostTt = Math.floor(Math.random() * w * 3.5) + 1000;
        ghostFading = false;
        ghostTs = new Date().getTime();
    ...
    function flyGhost() {
        ...
        if (dt > ghostTt * 3 / 4 && !ghostFading) {
            ghostFlying.fadeOut(Math.floor(ghostTt / 4));
            ghostFading = true;
        }
        var x = ghostX1 + Math.floor((ghostX2 - ghostX1) * dt / ghostTt);
        var y = ghostY1 + Math.floor((ghostY2 - ghostY1) * dt / ghostTt);
        ghostFlying.css('left', x).css('top', y);
        requestAnimationFrame(flyGhost);
    }
